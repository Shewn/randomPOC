...
export var VX = {
  ...
  ///point constructor
  Point: function (coordinates, materiality = "material") {
    // materiality can be "material" or "immaterial"
    VX.pointCount += 1;
  ...
  Span: function (point1, point2) {
    VX.spanCount += 1;
    if (Number.isInteger(point1)) {
      this.p1 = VX.getPoint(point1);
    } else {
      this.p1 = point1;
    }
    if (Number.isInteger(point2)) {
      this.p2 = VX.getPoint(point2);
    } else {
      this.p2 = point2;
    }
  ...
  addSkin: function (pointsArray, stylesObject) {
    ...
    for (var i = 0; i < pointsArray.length; i++) {
      for (var j = 0; j < VX.points.length; j++) {
        var pointId = Number.isInteger(pointsArray[i])
          ? VX.getPoint(pointsArray[i]).id
          : pointsArray[i].id;
  ...
  removePoint: function (id) {
    for (var i = 0; i < VX.points.length; i++) {
      if (VX.points[i].id == id) {
        VX.points.splice(i, 1);
      }
    }
  },
  ...
  updatePoints: function () {
    for (var i = 0; i < VX.points.length; i++) {
      ...
      if (!p.fixed) {
        ...
        if (VX.dimensions == "3d") {
          p.pz = p.cz;
        } // updates previous z as current z
        //applies skidloss
        if (VX.dimensions == "3d") {
          if (VX.yRange.min != null && p.cy <= VX.yRange.min + p.width / 2) {
            xv *= VX.skidLoss;
            zv *= VX.skidLoss;
          }
        } else if (VX.dimensions == "2d") {
          if (VX.yRange.max != null && p.cy >= VX.yRange.max - p.width / 2) {
            xv *= VX.skidLoss;
          }
        }
        //applies velocity
  ...
  updateSpans: function (currentRefinementIteration) {
    for (var i = 0; i < VX.spans.length; i++) {
      var refinementIterationsNeededForCurrentSpan = Math.round(
        VX.rigidity * VX.spans[i].strength
      );
      if (
        currentRefinementIteration <= refinementIterationsNeededForCurrentSpan
      ) {
        var s = VX.spans[i];
        ...
        if (!s.p1.fixed) {
          ...
          if (VX.dimensions == "3d") {
            s.p1.cz = mz - oz;
          } // updates span's first point z value
        }
        if (!s.p2.fixed) {
          ...
          if (VX.dimensions == "3d") {
            s.p2.cz = mz + oz;
          } // updates span's second point z value
        }
      }
    }
  },
  ...
  refinePositions: function () {
    ...
    for (var i = 0; i < VX.spans.length; i++) {
      ...
      if (
        refinementIterationsNeededForCurrentSpan >
        maxRequiredRefinementIterations
      ) {
        maxRequiredRefinementIterations =
          refinementIterationsNeededForCurrentSpan;
      }
  ...
  renderPoints: function () {
    if (VX.medium == "canvas") {
      ...
      if (!VX.svgPointsAddedToDOM) {
        for (var i = 0; i < VX.points.length; i++) {
          var p = VX.points[i];
          var pointCircle = document.createElementNS(
  ...
  renderSpans: function () {
    if (VX.medium == "canvas") {
      ...
      if (!VX.svgSpansAddedToDOM) {
        for (var i = 0; i < VX.spans.length; i++) {
          var s = VX.spans[i];
          var spanPath = document.createElementNS(
            "http://www.w3.org/2000/svg",
            "path"
          );
    ...
    if (VX.medium == "canvas") {
      for (var i = 0; i < VX.skins.length; i++) {
        ...
        for (var j = 1; j < s.points.length; j++) {
          VX.ctx.lineTo(s.points[j].cx, s.points[j].cy);
        }
      ...
      if (!VX.svgSkinsAddedToDOM) {
        for (var i = 0; i < VX.skins.length; i++) {
          var s = VX.skins[i];
          var skinPath = document.createElementNS(
            "http://www.w3.org/2000/svg",
            "path"
          );
  ...
  initialize: function (
    ...
    if (VX.dimensions == "2d") {
      ...
      if (VX.medium == "canvas") {
        ...
        VX.svg.setAttribute(
          "viewBox",
          `0 0 ${VX.interfaceWidth} ${VX.interfaceHeight}`
        );
